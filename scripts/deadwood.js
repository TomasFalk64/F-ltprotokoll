...
function woodRowHtml(row, index) {
  return `<div class="wood-row" data-wood-row="${index}"><div class="wood-species"><label>Trädslag${woodSelect('tradslag', woodSpecies, row.tradslag, 'Välj trädslag…')}</label><label${row.tradslag === 'Annat' ? '' : ' hidden'}>Ange trädslag<input data-wood-field="annatTradslag" placeholder="Exempelvis hassel eller hägg" value="${escapeHtml(row.annatTradslag)}"></label></div><div class="wood-columns">${Object.entries(woodLabels).map(([key, label]) => key === 'karaktar' ? `<div class="wood-character"><span class="wood-label">${label}</span><details><summary>${escapeHtml(woodCharacterSummary(row))}</summary><div class="wood-character-options">${woodCharacters(row.tradslag).map(item => `<label class="choice"><input type="checkbox" data-wood-field="karaktar" value="${escapeHtml(item)}"${row.karaktar.includes(item) ? ' checked' : ''}><span>${escapeHtml(item)}</span></label>`).join('')}<label>Annan vedkaraktär<input data-wood-field="annat" placeholder="Exempelvis sprickor, svampangrepp eller insektsgångar" value="${escapeHtml(row.annat)}"></label></div></details></div>` : `<label>${label}${woodSelect(key, woodOptions[key], row[key])}</label>`).join('')}<button class="wood-remove" type="button" data-wood-remove aria-label="Ta bort trädslagsrad ${index + 1}" title="Ta bort trädslag">×</button></div></div>`;
}
function woodGroupHtml(id, title) {
  const hint = id === 'liggande' ? 'Lågornas läge, markkontakt, fuktighet, nedbrytning och variation i dimensioner' : 'Torrakor och högstubbar: höjd, dimensioner, håligheter, bark och solexponering';
  return `<fieldset class="dead-entry" data-wood-group="${id}"><legend>${title}</legend><input type="hidden" name="ved_${id}" value=""><div class="wood-rows">${woodRowHtml(emptyWoodRow(), 0)}</div><button type="button" class="button button-outline wood-add" data-wood-add>Lägg till trädslag</button><label>Kommentar – ${title.toLowerCase()}<textarea name="ved_${id}_kommentar" rows="3" placeholder="${hint}"></textarea></label></fieldset>`;
}
