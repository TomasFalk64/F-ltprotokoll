...
function normalizeFields(entries) {
  ...
  for (const [legacyName, textName] of [['polygonCrs','polygonGeojson'],['centerCrs','centerCoordinate']]) {
    const system = fields.find(([name]) => name === legacyName)?.[1]?.trim();
    if (!system) continue;
    const text = fields.find(([name]) => name === textName);
...
document.querySelectorAll('.image-input').forEach(input => {
  input.title = 'klicka för att byta/lägga in bild';
});
...
function saveDraft() {
  ...
  try {
    ...
    storageWarning = '';
    setStatus('Utkast sparat i denna webbläsare, inklusive tillagda bilder.');
  } catch {
    storageWarning = 'Utkastet kunde inte sparas fullständigt lokalt. Spara rapporten som fil innan du lämnar sidan.';
    setStatus('');
  }
}
function showImages() {
...
function restoreDraft() {
  try {
    let entries = JSON.parse(localStorage.getItem('faltrapport-draft') || '[]');
    ...
    entries = normalizeFields(entries);
    ...
    setStatus(entries.length || Object.keys(images).length ? 'Sparat utkast återställt.' : 'Utkast sparas i denna webbläsare. Spara en rapportfil för en egen kopia.');
  } catch {
    setStatus('Det lokala utkastet kunde inte läsas. Spara en rapportfil för att behålla nya uppgifter.');
  }
  showImages();
  updateProgress();
  updatePolygonStatus();
...
document.querySelectorAll('.image-input').forEach(input => input.addEventListener('change', async () => {
  ...
  let url;
  try {
    if (input.dataset.target === 'mapImage') {
      const png = await mapFileToPng(file);
      if (generation !== imageGeneration || version !== (imageVersions[input.dataset.target] || 0)) return;
      images.mapImage = png;
      showImages();
      saveDraft();
      setStatus('Kartbilden har lagts till som PNG (högst 1600 pixlar). För TIFF används alltid första bilden.');
      return;
    }
    ...
    showImages();
    saveDraft();
    setStatus('Bild tillagd i rapporten.');
  } catch (error) {
    if (generation === imageGeneration) setStatus(`Bilden kunde inte läsas. ${error.message}`);
  } finally {
