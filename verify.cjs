...
function setup(saved = {}, blocked = false) {
  ...
  for (const match of form.innerHTML.matchAll(/<(input|textarea|select)\b([^>]*)>/g)) {
    ...
    if (attributes.name) {
      ...
      controls.push({...attributes, tagName:match[1].toUpperCase(), options, value:decode(attributes.value || ''), type:attributes.type || match[1], checked:false});
    }
  }
...
assert(badDraft.node('#storageStatus').textContent.includes('kunde inte läsas'));
const unavailable = setup({}, true);
unavailable.run('saveDraft()');
assert(unavailable.node('#storageStatus').textContent.includes('kunde inte sparas'));
